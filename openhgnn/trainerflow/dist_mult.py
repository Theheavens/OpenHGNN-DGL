...
class DistMult(BaseFlow):
    ...
    def __init__(self, args):
        ...
        if hasattr(self.model, 'r_embedding'):
            ...
            self.r_embedding = self.model.r_embedding
        else:
        ...
        if self.args.mini_batch_flag:
            ...
            self.dataloader = dgl.dataloading.EdgeDataLoader(
                self.hg, train_eid_dict, sampler, device=self.device,
                    negative_sampler=NegativeSampler(self.hg, 5), batch_size=1024,
            ...
            self.train_eid_dict = {
                etype: self.hg.edges(etype=etype, form='eid')
                for etype in self.hg.canonical_etypes}
            self.negative_sampler = NegativeSampler(self.hg, 5)
    ...
    def loss_calculation(self, positive_graph, negative_graph, logits):
        ...
        loss = self.loss_fn(th.cat((p_score, n_score)), th.cat((p_label, n_label)))
        return loss
    ...
    def _full_train_setp(self):
        ...
        loss = self.loss_calculation(self.hg, negative_graph, logits) + 0.01 * reg_loss
        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer.step()
        return loss.item()
    ...
    def _test_step(self, split=None, logits=None):
        ...
        with th.no_grad():
            ...
            metric = self.evaluator(logits, self.r_embedding, self.train_dataset, self.val_dataset, self.test_dataset, hits=[1], eval_p='raw')
        # if split == 'val':
